...
Required env vars:
- `VERSION` (no leading `v`, example `0.9.0`)
- `ARCH` (`arm64`)
...
```bash
VERSION=0.9.0 ARCH=arm64 ./scripts/package_macos.sh build
VERSION=0.9.0 ARCH=arm64 ./scripts/package_macos.sh contract
./scripts/verify_macos_binary.sh dist/flutree-0.9.0-macos-arm64.tar.gz
```
